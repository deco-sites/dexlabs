 ...
 */
const isProfilePropKey = (key: string): key is ProfilesKeys => key in profiles.$inferInsert;
/**
 * Checks if `value` is of the same type as `profiles[key]`.
 */
const isProfilePropType = (key: ProfilesKeys, value: unknown): value is ProfileValue<typeof key> => typeof value === typeof profiles.$inferInsert[key];
interface Props {
    mode?: "create" | "delete";
    email?: string;
}
export async function loader({ mode, email }: Props, req: Request, { invoke }: AppContext) {
    ...
    if (mode === "create" && req.body) {
        ...
        formData.forEach((value, key) => isProfilePropKey(key) &&
            isProfilePropType(key, value) &&
            (newProfile[key] = value as any));
        // Insert newProfile into the database.
        await drizzle.insert(profiles).values(newProfile as typeof profiles.$inferInsert);
...
export default function ManageProfiles({ profiles = [] }: SectionProps<typeof loader>) {
    ...
    name="email" id="email" required class="border border-gray-300 rounded"/>
            ...
            return (<div class="flex gap-2 items-center">
                            ...
                            <form hx-post={profileDeleteUrl} hx-trigger="click" hx-target="closest section" hx-swap="outerHTML" class="w-4 h-4">
