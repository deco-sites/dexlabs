...
export default function ({ title, titleCaption, caption, cta, heroSections }: Props) {
    ...
    return <section class="bg-primary max-w-[1378px] md:rounded-[40px] mx-auto p-4 md:p-[100px]">
        ...
        <h3 class="text-left mx-auto text-secondary text-xl md:text-2xl font-normal">{caption || ""}</h3>
        <div class="flex items-center gap-3 flex-wrap justify-left py-10">
            {cta?.map((item) => (
                <a
                    key={item?.id}
                    id={item?.id}
